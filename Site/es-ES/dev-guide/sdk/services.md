...
| **`SearchSyntaxService`** | `char TokenPrefix { get; }` | El carácter que inicia un token de consulta de complementos, configurado en Configuración → General → Sistema. Léelo en lugar de guardar tu propia copia: el escáner del host extrae la palabra de la consulta por ese carácter y la entrega con el carácter incluido, así que una copia propia solo puede coincidir o no reclamar nada. |
