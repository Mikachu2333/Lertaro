...
| :--- | :--- | :--- |
| **`FuzzyMatchService`** | `bool IsMatch(string pattern, string text)`<br>`bool[]? GetHighlightMask(string text, string query)`<br>`double GetMatchScore(string text, string query)` | Executes the host's fzf fuzzy matching engine, calculates character-level highlight masks with multi-tier fallback, and exposes the host's match-quality score for consistent result ranking. A `query` is read with the search box's own parser, so `:` excludes, `?` inverts a term's precision, `|` means OR, and every other character — including `!` `^` `$` `'` — is ordinary text. Multi-word queries are ANDed. |
| **`SearchSyntaxService`** | `char TokenPrefix { get; }` | The single character that starts a plugin query token, as configured under Settings → General → System. Read it instead of storing your own copy: the host's scanner lifts a token word out of the query BY that character and hands it over with the character still attached, so a copy of your own can only ever agree or silently claim nothing. |
