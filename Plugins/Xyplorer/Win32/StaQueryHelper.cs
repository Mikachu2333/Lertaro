...
internal static class StaQueryHelper
{
    ...
    public static T Run<T>(Func<T> func, T fallback)
    {
        ...
        {
            IsBackground = true,
            Name = "XyWin32QuerySta"
        };
        thread.SetApartmentState(ApartmentState.STA);
        thread.Start();

        if (!done.Wait(Timeout))
            Logger.Log("[Xyplorer] Win32 query timed out; returning null.", LogLevel.Warn);
        else if (error != null)
            Logger.Log($"[Xyplorer] Win32 query failed: {error.Message}", LogLevel.Warn);
